...
//		exit('<pre>'.print_r($_SESSION['wpsc_checkout_saved_values'], true).'</pre>');
		...
		switch($this->checkout_item->type) {
			...
			$output = "<textarea title='".$this->checkout_item->unique_name."' class='text' id='".$this->form_element_id()."' name='collected_data[{$this->checkout_item->id}]".$an_array."' rows='3' cols='40' >".esc_html( $saved_form_data )."</textarea>";
			break;
			...
			case "checkbox":
				...
				if($options != ''){
					...
					foreach($options as $label=>$value){
						$output .= "<input type='hidden' title='".$this->checkout_item->unique_name."' id='".$this->form_element_id().$j."' value='-1' name='collected_data[{$this->checkout_item->id}][".$i."][".$j."]'/><input type='checkbox' title='".$this->checkout_item->unique_name."' id='".$this->form_element_id()."' value='".esc_attr( $value )."' name='collected_data[{$this->checkout_item->id}][".$i."][".$j."]'/> ";
						$output .= "<label for='".$this->form_element_id().$j."'>".esc_html( $label )."</label><br />";
						$j++;
					}
				}
			break;
			...
			case "delivery_country":
				if(wpsc_uses_shipping() && (get_option('use_billing_unless_is_shipping') != 1)) {
					...
					$output = "<input title='".$this->checkout_item->unique_name."' type='hidden' id='".$this->form_element_id()."' class='shipping_country' name='collected_data[{$this->checkout_item->id}]' value='". esc_attr( $_SESSION['wpsc_delivery_country'] )."' /><span class='shipping_country_name'>".$country_name."</span> ";
			...
			case "select":
				...
				foreach((array)$options as $label => $value){
					...
					$output .="<option ".$selected. " value='". esc_attr( $value )."'>". esc_html( $label )."</option>\n\r";
				}
				$output .="</select>";
				}
			...
			case "radio":
				...
				if($options != ''){
					...
					$output .= "<label for='".$this->form_element_id()."'>". esc_html( $label )."</label>";
			...
			case "coupon":
			default:
			  $country_data = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM `".WPSC_TABLE_CURRENCY_LIST."` WHERE `isocode` IN(%s) LIMIT 1", $_SESSION['wpsc_delivery_country'] ), ARRAY_A);
				...
				$output = "<input title='".$this->checkout_item->unique_name."' type='hidden' id='".$this->form_element_id()."' class='shipping_region' name='collected_data[{$this->checkout_item->id}]' value='".$_SESSION['wpsc_delivery_region']."' size='4' /><span class='shipping_region_name'>".esc_html( $region_name )."</span> ";

					}else{
						$output = "<input class='shipping_region' title='".$this->checkout_item->unique_name."' type='text' id='".$this->form_element_id()."' class='text' value='".esc_attr( $saved_form_data )."' name='collected_data[{$this->checkout_item->id}]".$an_array."' />";
