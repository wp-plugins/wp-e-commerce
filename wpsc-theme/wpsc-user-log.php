<?php
	/**
	 * The User Account Theme.
	 *
	 * Displays everything within the user account.  Hopefully much more useable than the previous implementation.
	 *
	 *@todo This basically shows 'screens' for each of the following: Purchase History, Your Details, Downloads.  Could argue that these should be separate templates.
	 *
	 * @package WPSC
	 * @since WPSC 3.8
	 */
 global $files, $seperator, $purchase_log, $col_count, $siteurl, $products, $links;
?>
<div class="wrap">
	<div class='user-profile-links'>
		<a href="<?php echo get_option('user_account_url'); ?>">Purchase History</a> | 
		<a href="<?php echo get_option('user_account_url').$seperator."edit_profile=true"; ?>">Your Details</a> | 
		<a href="<?php echo get_option('user_account_url').$seperator."downloads=true"; ?>">Your Downloads</a>
	</div>
	<br />
<?php if ( is_wpsc_profile_page() ) : ?>

	<form method='post'>
		<?php 
			//This will print either the "Thanks" message or the "error" message.  Now filterable via 'wpsc_profile_message'
			echo validate_form_data(); 
		?>
	<table>
		<?php 
			// This function displays each of the form fields.  Each of them are filterable via 'wpsc_account_form_field_$tag' where tag is permalink-styled name or uniquename.
			// i.e. First Name under Shipping would be 'wpsc_account_form_field_shippingfirstname' - while Your Billing Details would be filtered 
			// via 'wpsc_account_form_field_your-billing-details'.
			 wpsc_display_form_fields();
		?>
		<tr>
		  <td>
		  </td>
		  <td>
		  <input type='hidden' value='true' name='submitwpcheckout_profile' />
		  <input type='submit' value='<?php _e('Save Profile', 'wpsc');?>' name='submit' />
		  </td>
		</tr>
	</table>
	</form>

<?php elseif ( is_wpsc_downloads_page() ) : ?>
	<?php if ( wpsc_has_downloads() ) : ?>
	<table class='logdisplay'>
			<tr>
				<th><?php _e('File Names', 'wpsc'); ?> </th>
				<th><?php _e('Downloads Left', 'wpsc'); ?> </th>
				<th><?php _e('Date', 'wpsc'); ?> </th>
			</tr>
			  <?php
				$i=0;
				foreach( (array)$files as $file ) :
				  $alternate = "";
				  if ( ( $i % 2 ) != 1 ) {
					$alternate = "class='alt'";
				  }
			  ?>
				<tr <?php echo $alternate; ?>>
					<td>
						<?php
						  if ($products[$i]['downloads'] > 0) {
							echo "<a href = ".$links[$i].">".$file['post_title']."</a>";
						  } else {
							echo $file['post_title']."";
						  }
						?>
					</td>
					<td><?php echo $products[$i]['downloads']; ?></td>
					<td><?php echo date(get_option("date_format"), strtotime($products[$i]['datetime']));?></td>
				</tr>
					<?php
						  $i++;
						endforeach;
					?>
		</table>
		<?php 
				else : _e('You have not purchased any downloadable products yet.', 'wpsc'); 
			endif;
		?>
	<?php 
		else : 
		// We're on the User Detail Page
	?>
		<?php if ( is_user_logged_in() ) : ?>
			
			<?php if ( wpsc_has_purchases() ) : ?>
			
				<?php if ( wpsc_has_purchases_this_month() ) : ?>
				<table class='logdisplay'>
					<tr class='toprow'>
						<td><strong><?php _e('Status', 'wpsc');?></strong></td>
						<td><strong><?php _e('Date', 'wpsc');?></strong></td>
						<td><strong><?php _e('Price', 'wpsc');?></strong></td>
						<?php        
							if ( get_option('payment_method') == 2 ) : 
						?>
						<td><strong><?php _e('Payment Method', 'wpsc');?></strong></td>
						<?php endif; ?>
					</tr>
					<?php wpsc_user_details(); ?>
				</table>
				<?php else /*if there have been no purchases this month*/: ?>
				<tr>
					<td colspan='<?php echo $col_count; ?>'>
						<?php _e('No transactions for this month.', 'wpsc'); ?>
					</td>
				</tr>
				<?php endif; ?>
				</table>
			<?php else /*if there have been no purchases at all*/ : ?>
				<table> 
					<tr>
						<td>     
							<?php _e('There have not been any purchases yet.', 'wpsc'); ?>
						</td>
					</tr>
				</table>
			<?php endif; ?>
			
		<?php else /*if user is not logged in*/ : ?>
			<?php_e('You must be logged in to use this page. Please use the form below to login to your account.', 'wpsc'); ?>
				<form name="loginform" id="loginform" action="<?php echo $siteurl; ?>/wp-login.php" method="post"><br />
					<label>Username:<br /><input type="text" name="log" id="log" value="" size="20" tabindex="1" /></label><br />
					<label>Password:<br /> <input type="password" name="pwd" id="pwd" value="" size="20" tabindex="2" /></label>
					<p>
					<label><input name="rememberme" type="checkbox" id="rememberme" value="forever" tabindex="3" /> 
					Remember me</label></p>
					<p class="submit">
					<input type="submit" name="submit" id="submit" value="Login &raquo;" tabindex="4" />
					<input type="hidden" name="redirect_to" value="<?php echo get_option('user_account_url'); ?>" />
					</p>
				</form>
		<?php endif; ?>
		
	<?php endif; ?>
</div>